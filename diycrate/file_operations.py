...
def watch_wrapper(box_dir, trash_dir, oauth, app, wait_time):
    for changes in watch(box_dir, trash_dir):
        for change in changes:
            change_type, file_path = change
            crate_logger.info("Change type: %s, file path: %s", change_type, file_path)
...
class WatchEventHandler(object):
    ...
    def __init__(self, *args, **kwargs):
        ...
        our_kargs = {k: kwargs.get(k) for k in our_keys}
        for key in our_keys:
            kwargs.pop(key, None)
    ...
    def process_event(self, event: FileChange, operation: str):
        """
    ...
    def process_real_close_event(self, event: FileChange):
        file_path = Path(event[1])
        ...
        try:
            is_base = any(
                Path(evp).relative_to(BOX_DIR) for evp in [event[1], event[1][:-1]]
            )
        except ValueError:
            is_base = False
        ...
        last_dir = Path(event[1]).parent.name
        if not is_base:
            AssertionError(
        ...
        offset = 0
        safe_iter = SafeIter(
            cur_box_folder.get_items(offset=offset, limit=limit), path=event[1]
        )
    ...
    def process_modify_event(self, event: FileChange, operation: str):
        file_path = Path(event[1])
        crate_logger.debug(f"{operation}...: {file_path.as_posix()}")
        if r_c.exists(f"{event[1]}:created:enqueue"):
            crate_logger.info(
    ...
    def process_create_event(self, event: FileChange):
        crate_logger.debug("Creating: {}".format(event[1]))
        folders_to_traverse = self.folders_to_traverse(event[1])
        client = Client(self.oauth)
        failed = False
        ...
        did_find_the_file = os.path.isdir(event[1])  # true if we are a directory :)
        did_find_the_folder = os.path.isfile(
            event[1]
        )  # true if we are a regular file :)
        is_file = os.path.isfile(event[1])
        is_dir = os.path.isdir(event[1])
        limit = 100
        offset = 0
        for entry in cur_box_folder.get_items(offset=offset, limit=limit):
            did_find_the_file = (
                is_file
                and entry["type"] == "file"
                and entry["name"] == os.path.basename(event[1])
            )
            did_find_the_folder = (
                is_dir
            ...
            if did_find_the_file:
                ...
                break
        if is_file and not did_find_the_file:
            crate_logger.debug("Upload the file: {}".format(event[1]))
            last_modified_time = os.path.getctime(event[1])
            r_c.set(f"{event[1]}:created:enqueue", value=1, ex=timedelta(minutes=1))
            queue_item = UploadQueueItemReal(
                datetime.fromtimestamp(last_modified_time)
                .astimezone(dateutil.tz.tzutc())
                .timestamp(),
            ...
            crate_logger.debug("Upload the folder: {}".format(event[1]))
            queue_item: UploadQueueItem = partial(
    ...
    def process_move_event(self, event: Tuple[FileChange, FileChange]):
        crate_logger.debug("Doing a move on: {}".format(event))
        src_event, dest_event = event
        folders_to_traverse = self.folders_to_traverse(dest_event[1])
        client = Client(self.oauth)
        failed = False
        box_folder = None
        while not failed:
        ...
        is_file = os.path.isfile(dest_event[1])
        is_dir = os.path.isdir(dest_event[1])
        ...
        for entry in safe_iter:
            ...
            if did_find_src_file:
                ...
                if is_rename:
                    crate_logger.debug(f"Processing as a rename {dest_event[1]=}")
                    last_modified_time = (
                        datetime.fromtimestamp(Path(dest_event[1]).stat().st_mtime)
                        .astimezone(dateutil.tz.tzutc())
                        .timestamp()
                    )
                    ...
                    r_c.set(
                        local_or_box_file_m_time_key_func(dest_event[1], False),
                        datetime.fromtimestamp(Path(dest_event[1]).stat().st_mtime)
                        .astimezone(dateutil.tz.tzutc())
                        .timestamp(),
                    )
                    ...
                    version_info["file_path"] = dest_event[1]
                    version_info["etag"] = file_obj["etag"]
                    r_c.set(
                        redis_path_for_object_id_key(dest_event[1]),
                        src_file.object_id,
                    )
                    r_c.delete(
                        redis_path_for_object_id_key(
                    ...
                    crate_logger.debug(f"Processing as a move {dest_event[1]=}.")
                    ...
                    for cur_entry in cur_box_folder_items:
                        ...
                        if did_find_cur_file:
                            crate_logger.info(
                                f"Did find cur file (moving to {dest_event[1]=})"
                            )
                            queue_item = UploadQueueItemReal(
                                datetime.fromtimestamp(
                                    Path(dest_event[1]).stat().st_mtime
                                )
                                .astimezone(dateutil.tz.tzutc())
                                .timestamp(),
                                partial(cur_entry.update_contents, dest_event[1]),
                                self.oauth,
                                dest_event[1],
                            )
                    ...
                    if is_file and not did_find_cur_file and did_find_src_file:
                        crate_logger.debug(
                            f"Did not find cur file {dest_event[1]=} "
        ...
        if (
            ...
            if not move_from_remote:
                if is_file and not did_find_src_file:
                    ...
                    last_modified_time = Path(dest_event[1]).stat().st_mtime
                    queue_item = UploadQueueItemReal(
                        datetime.fromtimestamp(last_modified_time)
                        .astimezone(dateutil.tz.tzutc())
                        .timestamp(),
                    ...
                    # wm.add_watch(dest_event[1], rec=True, mask=mask, auto_add=True)

    def process_delete_event(self, event: FileChange):
    ...
    def process_IN_CREATE(self, event: FileChange):
        """
        Overrides the super.
        :param event:
        :return:
        """
        if not os.path.basename(event[1]).startswith(
            ".~lock"
        ):  # avoid propagating lock files
            self.incoming_operations.put([event, "create"])

    # noinspection PyPep8Naming
    def process_IN_DELETE(self, event: FileChange):
        """
        Overrides the super.
        :param event:
        :return:
    ...
    # noinspection PyPep8Naming
    def process_IN_MODIFY(self, event: FileChange):
        """
        Overrides the super.
        :param event:
        :return:
    ...
    # noinspection PyPep8Naming
    def process_IN_MOVED_FROM(self, event: FileChange):
        """
        Overrides the super.
        :param event:
        :return:
        ...
        if not os.path.basename(event[1]).startswith(
            ...
            crate_logger.debug("Moved from: {}".format(event[1]))
            self.move_events.append(event)

    # noinspection PyPep8Naming
    def process_IN_MOVED_TO(self, event: FileChange):
        """
        Overrides the super.
        :param event:
        :return:
        ...
        crate_logger.debug(event)
        found_from = False
        to_trash = trash_directory == Path(event[1]).parent
        to_box = BOX_DIR in Path(event[1]).parents
        ...
        for move_event in self.move_events:
            ...
            if (
                ...
                and was_moved_from
                and BOX_DIR in Path(move_event[1]).parents
            ):
                found_from = True
        ...
        if not found_from and (not to_trash and to_box):
            ...
            crate_logger.debug("Moved to: {}".format(event[1]))

    # noinspection PyPep8Naming,PyMethodMayBeStatic
    def process_IN_DELETE_SELF(self, event: FileChange):
        crate_logger.debug(event)

    # noinspection PyPep8Naming,PyMethodMayBeStatic
    def process_ALL_EVENTS(self, event: FileChange):
        crate_logger.debug(event)

    # noinspection PyPep8Naming
    def process_IN_CLOSE(self, event: FileChange):
        """
        Overrides the super.
        :param event:
        :return:
        ...
        if not os.path.basename(event[1]).startswith(
            ...
            crate_logger.debug(f"Skipping close on: {event[1]=} due to `.~lock`")
...
class EventHandler(pyinotify.ProcessEvent):
    ...
    def traverse_path(
        ...
        for folder in folders_to_traverse:
            ...
            if not did_find_folder:
                try:
                    cur_box_folder = cur_box_folder.create_subfolder(folder).get()
                    crate_logger.debug(f"Sub-folder creation: {event[1]}")
                except BoxAPIException:
                    crate_logger.debug(
                        "Box exception as we try to create a sub-folder {folder}".format(
                            folder=folder
    ...
    def process_modify_event(self, event: FileChange, operation: str):
        ...
        if is_file and not did_find_the_file:
            crate_logger.debug("Uploading contents...: {}".format(file_path.as_posix()))
            last_modified_time = Path(event[1]).stat().st_mtime
            queue_item = UploadQueueItemReal(
                datetime.fromtimestamp(last_modified_time)
                .astimezone(dateutil.tz.tzutc())
                .timestamp(),
    ...
    def process_move_event(self, event: FileChange):
        crate_logger.debug("Doing a move on: {}".format(event))
        src_event, dest_event = event
        folders_to_traverse = self.folders_to_traverse(dest_event[1])
        client = Client(self.oauth)
        ...
        offset = 0
        folder_gen = src_box_folder.get_items(offset=offset, limit=limit)
        safe_iter = SafeIter(folder_gen, path=dest_event[1])
        for entry in safe_iter:
            if entry is None:
                continue
            did_find_src_file = (
                is_file
                and entry["name"] == Path(src_event[1]).name
                and entry["type"] == "file"
            )
            did_find_src_folder = (
                is_dir
            ...
            if did_find_src_file:
                ...
                if is_rename:
                    ...
                    src_file.rename(Path(dest_event[1]).name)
                    file_obj: File = client.file(file_id=src_file.object_id).get()
                    version_info["file_path"] = dest_event[1]
                    version_info["etag"] = file_obj["etag"]
                    r_c.set(
                    ...
                    r_c.delete(
                        redis_path_for_object_id_key(
                            Path(src_event[1]) / Path(src_file[1]).name
                        )
                    )
                    r_c.set(redis_key(src_file.object_id), json.dumps(version_info))
                    r_c.set("diy_crate.last_save_time_stamp", int(time.time()))
                    ...
                    if is_file and not did_find_cur_file and did_find_src_file:
                        ...
                        # src_file.move(cur_box_folder)
                        # do not yet support moving and renaming in one go
                        assert src_file["name"] == Path(dest_event[1]).name
            elif did_find_src_folder:
                src_folder = client.folder(folder_id=entry["id"]).get()
                if is_rename:
                    src_folder.rename(Path(dest_event[1]).name)
                else:
                    src_folder.move(cur_box_folder)
                    # do not yet support moving and renaming in one go
                    assert src_folder["name"] == Path(dest_event[1]).name
            elif entry["name"] == Path(dest_event[1]).name:
                if (
                    Path(src_event[1]) == Path(dest_event[1])
                    and (
                        Path(Path(src_event[1]).name).stem == Path(dest_event[1]).name
                        or (
                            Path(Path(src_event[1]).name).suffix == ".tmp"
                            and Path(Path(dest_event[1]).name).suffix != ".tmp"
                        )
                    )
                    and Path(src_event[1]).name != Path(dest_event[1]).name
                ):
                    crate_logger.info(
                        f"Detected conceptual same directory temp file rename from "
                        f"{Path(src_event[1]).name=} to {Path(dest_event[1]).name=}"
                    )
                    cur_offset = 0
                    cur_entry: Union[boxsdk.object.file.File, boxsdk.object.file.Item]
                    for cur_entry in cur_box_folder.get_items(
        ...
        if (
            ...
            offset = 0
            for entry in cur_box_folder.get_items(offset=offset, limit=limit):
                if entry["name"] == Path(dest_event[1]).name:
                    move_from_remote = True
                    break
            if not move_from_remote:
                if is_file and not did_find_src_file:
